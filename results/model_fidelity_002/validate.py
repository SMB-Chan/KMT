"""Native 10-cond takeoff/landing after ISA-on-1D and height-scaled gusts."""
import hashlib
import json
import math
import sys
...
def run_case(scenario, seed, wind, Hs, duration):
    ...
    return dict(scenario=scenario, seed=seed, wind_y=wind, Hs=Hs, status=status,
                t=v.t, z=v.z, Vx=v.Vx, Vz=v.Vz, y=v.y, T_factor=snap['T_factor'])
    ...
    for scenario, Hs, duration in (('takeoff', 0.3, 15.0), ('takeoff', 1.5, 15.0),
        ...
        for seed in SEEDS:
            for wind in WINDS:
                ...
                rows.append(row)
                print(scenario, Hs, seed, wind, row['status'], round(row['t'], 2), flush=True)
    grouped = {}
    for row in rows:
        grouped.setdefault(f"{row['scenario']}_Hs{row['Hs']}", []).append(row)
    report = dict(
        note='native controller after ISA-on-1D and height-scaled gusts; hull drag unchanged',
