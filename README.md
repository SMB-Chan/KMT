...
| `typhoon_forecast.py` | 気象庁台風通報の実戦予報（進路VAR・強度緩和・パラメトリック降水場・渦風、シミュレータ設定生成） |
| `typhoon_forecast_study.py` | 台風25号(ドゥージェン)の実戦予報と検証（対JMA公式予報・AMeDAS観測・運航可否判断） |
| `fetch_typhoon_dujuan.py` | 気象庁の台風電文・諸元XML・府県予報R1・AMeDASの取得（`data/jma_typhoon_2625/`） |
...
| `data/` | 取得した NOAA NDBC データ（リアルタイム・スペクトル）と気象庁台風通報（`jma_typhoon_2625/`） |
...
### 台風の実戦予報（typhoon_forecast.py）

リプレイもVAR予報も「単一ブイの点」を扱いますが、`typhoon_forecast.py` は
**気象庁の気象通報（台風電文・諸元・府県予報・AMeDAS）を出典**に、実在する
台風25号(2026年・ドゥージェン, `TC2630`)の進路・強度・降水を実運用の形で予報し、
運航可否まで判定します。入力は `fetch_typhoon_dujuan.py` が取得して
`data/jma_typhoon_2625/` に固定した気象庁製品だけです（ネットワークへは行きません）。

| 出典 | 内容 |
|---|---|
| 台風電文（実況・予報円） | 中心位置・進行方向・速度、暴風域/強風域の方位別半径 |
| 諸元 XML（公式予報点） | JMA が公表した lead 1〜43 h の予報位置・強度・円 |
| 府県天気予報 R1 XML | 降水予報「多い所で」（1h/24h）・降水確率（POP） |
| AMeDAS | 実測の1時間・3時間・24時間降水、風、気温（検証にのみ使用） |

固定スナップショットは台風電文 **133 報**・軌跡 **40 点**（発生 09/16 21:00 JST〜
実況最終 09/21 08:00 JST、107 h）・AMeDAS **1008 記録**（72 時刻×14 観測所）、
取得時刻 2026-09-21T00:32:36Z。再取得はしません。

- **進路**: 3 時間格子に再標本化した (緯度, 経度) へリッジ VAR（次数 3・窓 36 h）を
  フィットし、発行時刻の実況を初期条件に再帰予測します。コンパニオン行列の
  スペクトル半径 < 1 を保証するため予報は有界で、lead 0 は実況そのものです。
- **強度**: 中心気圧は直近の傾向で予報し、温帯低気圧化（30〜40°N）へ緩和。
  最大風速は気圧–風速関係（通報が支持する指数 ≈0.58、既定 0.5）で実況にアンカーします。
- **降水**: パラメトリックな雨域（強風域半径の `RAIN_DECAY_FRAC` 倍を e-folding、
  移動方向へ `RAIN_LEAD_FRAC` 倍だけ変位＝進行方向が濡れる）、振幅は中心気圧欠損に
  比例し地形アンカーで増幅。JMA「多い所で」にアンカーした製品も生成します。
- **風**: 暴風域/強風域の 2 半径が冪乗則を過定決定（解析値で b≈0.47・rmax≈91 km）、
  非対称な渦風＋流入。瞬間風速/最大風速比（通報由来）でガスト RMS を校正します。
- `configs_at` / `typhoon_configs` / `forecast_configs` は (WeatherConfig, AtmosphereConfig)
  を返し、気象層が消費するのと同じ形で台風スナップショットが大気を駆動できます。
  すべて解析的・決定論的です。

```bash
# 気象庁通報の取得（data/jma_typhoon_2625/ へ固定。通常は再取得不要）
python3 fetch_typhoon_dujuan.py

# 実戦予報と検証スタディ（進路・強度・降水・運航可否、対 JMA 公式予報）
python3 typhoon_forecast_study.py     # -> results/typhoon_25_001/
```

検証結果は `results/typhoon_25_001/REPORT.md`：

- **進路 hindcast**（28 発行・リード 3〜24 h・回転起点、発行より後の実況のみを
  答えとする）で VAR RMSE **171 km** < 移動永続 209 km < 永続 303 km。
  永続比スキル +0.34〜+0.59、移動永続比 +0.02〜+0.20。
- **強度**は公式予報 9 点との平均絶対差 気圧 3.2 hPa・風速 1.5 m/s。
- **降水 vs AMeDAS**（1008 点）RMSE 3.87 mm/h・バイアス +0.07・相関 0.51・CSI 0.343。
  **vs 通報「多い所で」**（19 地方窓）中央比 0.51、POP 相関 0.83・CSI 0.492。
- **運航可否**（`weather.PRESETS['rain']` の制限）は製品トラックで go 40%
  （自由運転 VAR 35%）・一致 94%・律速は風。公式予報窓（+43 h）内は go 0%・
  一致 99.5% で、嵐の接近中は飛ばない判断が一致します。陸上風を 0.305 倍に
  緩めると go 96%（外洋が律速）。降水を JMA アンカーへ上げると雨の違反が 0→33 件。

予報は発行時刻より前の通報だけで学習し、公式予報と AMeDAS は検証にのみ使います。
素のパラメトリック降水場は分布（どこが濡れるか）を再現する一方、ピーク量は
「多い所で」の約半分（中央比 0.51）で、JMA アンカーは中央値 ×3.26 を要しました。
