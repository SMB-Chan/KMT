...
def integrate(ac, hull, hd, atmosphere, surface_at, state, *, dt, t,
    ...
    `weather` is an optional weather.Weather object (normally the same
    instance passed as `atmosphere`). Its rain/icing penalties -- propeller
    wetting, roughness drag and a lowered CL_max cap -- are evaluated once
    per call at the initial state and held over the substeps (steady
    weather); the accumulated ice mass itself flows through `extra_mass`.
    With weather=None the integration is bitwise identical to before.
    """
    ...
    if weather is not None:
        wind0 = atmosphere.wind(t, z)
        speed0 = float(np.linalg.norm(np.array([vx, vy, vz]) - wind0))
        wx = weather.effects(t, z, speed0, area=ac.geom.S)
        if not 0.0 < wx.prop_factor <= 1.0 or wx.extra_cd < 0.0 \
                or not 0.0 < wx.cl_max_factor <= 1.0:
            raise ValueError('invalid weather effects')
    ...
    for i in range(n_sub):
        ...
        cl = ac.CL(alpha)
        if wx is not None:
            # Ice roughness caps CL_max and adds parasite drag; rain and ice
            # foul the propeller. Guarded so weather=None stays bitwise.
            cap = ac.aero.CL_max * wx.cl_max_factor
            cl = float(np.clip(cl, -cap, cap))
            lift = q * ac.geom.S * cl
            drag = q * ac.geom.S * (ac.CD(cl, height_m=z - surface)
                                    + wx.extra_cd + 0.3 * beta ** 2)
            thrust = (ac.prop.thrust(speed, throttle, rho=density)
                      * thrust_factor * wx.prop_factor)
        else:
            lift = q * ac.geom.S * cl
...
def advance(env, x, z, vx, vz, pitch, throttle, sea, t,
    ...
    state, forces = integrate(
        ...
        current=env.cfg.current, extra_mass=extra_mass, weather=weather)
