...
重量 W = m·g = 80.0 × 9.80665 = **784.53 N**（設計点）
...
| **推力／重量比** | **0.70** | — | 静推力時に浮力補助が必須 |
| 失速速度 V_stall | 6.4 | m/s | — |
| 巡航速度 V_cruise | 11.0 | m/s | V_stall × √3 |
| 最大揚抗比 L/D_max | 16.3 | — | CL\* = √(CD0·π·e·AR) で算出 |

### 1.4 既知の制約

- 縦運動のみモデル化。**横・回転・独立舵面操作は未モデル。**
- 推力係数 0.70 のため、**離水は船体浮力を補助として成立する設計**です。
  `validate.py` の物理検証は "T_static < W" を通知しています。
- 風擾乱は含みません。外乱源は海面のみです。
